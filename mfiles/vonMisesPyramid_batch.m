function [pyr1, msk1, pyr2, msk2] = vonMisesPyramid_batch(map, vmPrs)
...
%             pyr1(l).orientation(ori).data = imfilter(map(l).data,msk_1,0);
            pyr1(l).orientation(ori).data = imfilter(map(l).data,vmPrs.msk_1{ori},0);
...
%             pyr2(l).orientation(ori).data = imfilter(map(l).data,msk_2,0);
            pyr2(l).orientation(ori).data = imfilter(map(l).data,vmPrs.msk_2{ori},0);
