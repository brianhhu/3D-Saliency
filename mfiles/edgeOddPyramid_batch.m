...
function newMap1 =  edgeOddPyramid_batch(map,params)
...
%     Oddmsk1 = makeOddOrientationCells_batch(prs.oris(ori),prs.lambda,prs.sigma,prs.gamma);
...
%         newMap1(l).orientation(ori).data = validFilt(map(l).data,Oddmsk1);
        newMap1(l).orientation(ori).data = validFilt(map(l).data,prs.Oddmsk1{ori});
